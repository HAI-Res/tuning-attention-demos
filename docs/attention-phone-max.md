...
> Long-form notes on the phone-side Max patches — the rationale behind them. The run sheet is [`patches/max/README.md`](../patches/max/README.md). The `cv.channel` / 7401 receiver described near the end was an earlier camera design that never got a sender; the camera patches that exist are in [`attention-cv-max.md`](attention-cv-max.md).
